...
export async function POST(req: Request) {
	...
	await dbConnect();
	try {
		const newData = await Product.create(body);
...
export async function PUT(req: Request) {
	...
	if (isDev) {
		const updated = dev.update("products", _id, updates);
	...
	await dbConnect();
	try {
		const updated = await Product.findOneAndUpdate({ _id, clerkId: adminData.clerkId }, updates, { new: true });
		if (!updated) return NextResponse.json({ error: "Not found" }, { status: 404 });
		return NextResponse.json(updated);
	} catch {
...
export async function DELETE(req: Request) {
	...
	if (isDev) {
		const removed = dev.remove("products", id);
	...
	await dbConnect();
	try {
		const deleted = await Product.findOneAndDelete({ _id: id, clerkId: adminData.clerkId });
